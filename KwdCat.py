...
Name = "Python Kenwood CAT library"
Version = "0.1"
VersionDate = "18/01/2023"
...
class KwdCat(object):
    ...
    def open_port(self,port='COM1',baudrate=57600,bytesize=8,stopbits=1,parity='N',xonxoff=0,rtscts=0,dsrdtr=0,dtr=1,rts=1,timeout=0) -> bool:
        #
        # opens a COM port. If a different value is needed that default values
        # they must be given when calling
        #
        self.port = port
        ...
        try:
            ...
            self.serial.dtr = self.dtr
            self.serial.rts = self.rts
    ...
    def send2port(self,datastosend:str):
        try:
    ...
    def query(self, request: str,length:int) -> str:
        ...
        ########################################
            ...
            send_string = f"{request.strip()};"                 # remove whitespaces if any and apprend terminator ;
            try:
...
if __name__ == "__main__":
  print ("%s - (c) Patrick EGLOFF aka TK5EP" %(Name))
